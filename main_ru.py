...
if __name__ == '__main__':
    ...
    print(f"Минимальная сумма для отправки = {gas_amount[0]} {gas_token} + 10%")    
    print(f"Максимальная сумма для отправки = {gas_amount[1]} {gas_token} - 10%")

    #print(f'Minimum Gas amount: {gas_amount} {gas_token} + 10%')
    print(f"Вы можете ввести значение >= {gas_amount[0]} {gas_token} <= {gas_amount[1]} {gas_token}")
    print (f"Вы также можете ввести 'min'/ 'max' для отправки минимальной/максимальной суммы {gas_token}")
    amount = ''
    while (amount != 'MAX') or (amount != 'MIN') or ((amount) < gas_amount[0]) or (float(amount) > gas_amount[1]):
     amount = input('Введите сумму отправки: ')
     amount = amount.upper()
    ...
    print(f"{amount} {gas_token} будет отправлено со всех ваших кошельков")
    w3 = Web3(Web3.HTTPProvider(rpc))
    w3.middleware_onion.inject(geth_poa_middleware, layer=0)
